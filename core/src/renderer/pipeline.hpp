...
public:
	...
	bool createLineVertPipeline(flecs::world& ecs, const char* name) {
		...
		pipeInfo.multisample_state.sample_mask = 0xFFFFFFFF;  // Use all samples
		...
		if (!pipeline) {
			SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Failed to create %s pipeline: %s", name, SDL_GetError());
			return false;
		}

		SDL_Log("Successfully created %s pipeline", name);
