...
export void check_error_bool(const bool res)
{
    if (!res) {
        LogError(LOG_APP, "%s", SDL_GetError());
    }
}
...
export glm::vec3 JPHVec3ToGLM(JPH::Vec3Arg vec) {

    return glm::vec3(vec.GetX(), vec.GetY(), vec.GetZ());
}

export JPH::Vec3 GLMVec3ToJPH(const glm::vec3 vec) {

    return JPH::Vec3(vec.x, vec.y, vec.z);
}


export glm::quat JPHQuatToGLM(JPH::QuatArg quat) {

    return glm::quat(quat.GetW(), quat.GetX(), quat.GetY(), quat.GetZ()); // w, x, y, z order      
}

export JPH::Quat GLMQuatToJPH(const glm::quat quat) {
...
export glm::vec3 quatToDirection(const glm::quat& q)
{
	// Rotate the default forward vector (-Z in OpenGL/GLM convention)
	return glm::normalize(q * glm::vec3(0.0f, 0.0f, -1.0f));
}

export JPH::Vec3 quatToDirection(const JPH::Quat& q)
{
...
export inline glm::quat rotationFromEulerDegrees(const glm::vec3& eulerDeg)
{
...
export JPH::Quat dirToQuat(const JPH::Vec3& dir) {
...
export namespace util {
	...
	export fs::path getRepoAssetsFolder() {
	...
	export fs::path getRepoRagdollsFolder() {
	...
	export fs::path getBuildAssetsFolder() {
	...
	export fs::path getBuildRagdollsFolder() {
	...
	export bool saveDataToFile(std::stringstream& dataOut, const std::filesystem::path& filePath) {
	...
	export uint64_t generateAssetID(std::string_view assetName) {

		return XXH3_64bits(assetName.data(), assetName.size());
	}

	/// Even with seed 0 this will produce a different hash than generateAssetID.
	export uint64_t generateAssetIDWithSeed(std::string_view assetName, uint64_t seed) {

		return XXH3_64bits_withSeed(assetName.data(), assetName.size(), seed);
	}

	export uint64_t generateRandomAssetID(std::string_view assetName) {
	...
	export uint64_t generateContentHash(const fs::path& filePath) {
